class base_noc_config extends uvm_object;
  ...
  function void parse_args();
    ...
    if($value$plusargs("AGENT_ACTIVE=%b", active));
    agent_mode = active ? UVM_ACTIVE : UVM_PASSIVE;
  endfunction

  function void print();
    uvm_report_info(get_type_name(), $sformatf("AGENT_ACTIVE = %b", agent_mode), UVM_NONE);
